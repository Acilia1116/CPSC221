...
// address operator
// &pixel returns the address of pointer(i.e. pointer of pixel object here)
// pixel returns the value of pointer (i.e. pointer of pixel object), which is address of pixel object
// we want to access the pixel object, we need the address of pixel, which is the value of pointer of pixel
    ...
    for (int i = -1; i < 2; i++){
        for (int j = -1; j < 2; j++){
            if (i == 0 && j == 0) {
                ...
                hslaColor hsla = rgb2hsl(rgba); // RGB->HSL conversion
                score += 8 * (hsla.l);
            } else {
